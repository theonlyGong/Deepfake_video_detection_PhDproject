...
class SCRFD():
    ...
    def detect(self, srcimg):
        ...
        # Windows系统直接outs = 模型前向推理就行。
        # outs = self.net.forward(self.net.getUnconnectedOutLayersNames())


        # Linux系统专属加入以下三行， windows直接删除下面三行！
        outs = []
        for i in range(3):
...
if __name__ == '__main__':
    ...
    img = cv2.imread("test_imgs/output.png")
    ...
    logger.info('Bounding box:({},{}),({},{})'.format(int(xmin),int(ymin),int(xamx),int(ymax)))
    cv2.rectangle(img, (int(xmin), int(ymin)), (int(xamx), int(ymax)), (0, 0, 255), thickness=2)
    cv2.imshow("FACIAL_LANDMARKS", img)
    cv2.waitKey()
