...
class SCRFD():
    ...
    def detect(self, srcimg):
        ...
        indices = cv2.dnn.NMSBoxes(bboxes.tolist(), scores.tolist(), self.confThreshold, self.nmsThreshold, top_k=1)
        if len(indices) != 1:
            print("many faces =", len(indices))
        for i in indices:
            # i = i[0]
            # xmin, ymin, xamx, ymax = int(bboxes[i, 0]), int(bboxes[i, 1]), int(bboxes[i, 0] + bboxes[i, 2]), int(
            #         bboxes[i, 1] + bboxes[i, 3])
            xmin, ymin, xamx, ymax = bboxes[i, 0], bboxes[i, 1], bboxes[i, 0] + bboxes[i, 2], bboxes[i, 1] + bboxes[
                i, 3]
            return [xmin, ymin, xamx, ymax]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='test1.jpg', help='image path')
    ...
    face_info = {}
    for video_name in os.listdir(r"E:\DeepFakeDetection\datasets\original_sequences\youtube\c23\videos"):
        # video_name = "016.mp4"
        capture = cv2.VideoCapture(
                r"E:\DeepFakeDetection\datasets\original_sequences\youtube\c23\videos" + "\\" + video_name)
        cur_face_info = []
        while True:
            # 获取一帧
            ...
            if not ret:
                break
            face_xy_list = mynet.detect(frame)
            cur_face_info.append(face_xy_list)
        face_info[video_name[:-4]] = cur_face_info
        logger.info(f"{video_name} transform over !")

    f = open('ffpp_face_rects.pkl', 'wb')
