...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, drop=0.2, attn_drop=0.2, drop_path=0.2,
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        global_feature = x
        global_feature = self.norm(global_feature)  # B L C
        global_feature = self.avgpool(global_feature.transpose(1, 2))  # B C 1
        global_feature = torch.flatten(global_feature, 1)
        global_feature = self.head(global_feature)
